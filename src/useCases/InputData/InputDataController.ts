...
export class InputDataController {
    ...
    handle(request: Request, response: Response): Response {
        ...
        if (!validateData(dataDecoded.decoded)) return response.status(500).json({ error: errors.invalidData })
        ...
        if (!extractedData.success) return response.status(500).json({ error: errors.invalidData })
        ...
        if (!result.success) return response.status(500).json({ error: errors.proccessError })
